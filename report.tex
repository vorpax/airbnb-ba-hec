...
\begin{titlepage}
  \begin{center}
    ...
    {\small\color{airbnbGrey}
      Data: Inside Airbnb (CC0 licence) $\cdot$ 64\,690 Paris listings (raw)\\
...
\begin{abstract}
\noindent
This report presents a data-driven analysis of Airbnb listing prices in Paris, drawing on a dataset of 64\,690 active listings from Inside Airbnb (filtered to 63\,520 after outlier removal). We address the central question: \textit{what are the key drivers of nightly prices, and can we build a reliable predictive model to help hosts set an optimal price?} Our pipeline combines exploratory data analysis, feature engineering (33 raw columns to 78 candidate features, then 37 selected), supervised regression and classification, amenity value analysis, and unsupervised segmentation. Across eight models, a GridSearch-tuned XGBoost pipeline achieves the strongest out-of-sample performance: R\textsuperscript{2}~=~0.657 on the log-price scale, with a back-transformed MAE of \euro{}27.2 on a median listing price of \euro{}80. Accommodation capacity (\texttt{accommodates}, \texttt{bedrooms}), room type, neighbourhood, and amenity count are the dominant price drivers. K-Means clustering (k~=~4) identifies four economically coherent market segments, independently validated by Ward hierarchical clustering (78\% agreement). These findings are translated into five actionable recommendations for Parisian hosts.
\end{abstract}
...
The dataset originates from \textit{Inside Airbnb} (\url{https://insideairbnb.com}), a public-interest project that periodically scrapes listing data under a Creative Commons CC0 licence. The raw file is a multi-city export (10 cities, \(\approx\)280\,000 rows); after filtering for Paris, the working dataset comprises \textbf{64\,690 listings} across 20 arrondissements, accompanied by \textbf{5.4 million review records} spanning 2010--2025.

This export differs from the standard Inside Airbnb schema in several ways that affect the analysis:
\begin{itemize}[itemsep=2pt]
  ...
  \item Several commonly available columns are absent: \texttt{beds}, \texttt{bathrooms}, \texttt{cancellation\_policy}, \texttt{availability\_365}, and \texttt{number\_of\_reviews}.
...
\begin{enumerate}[itemsep=5pt]
  ...
  \item \textbf{Seasonality.} Although the seasonal decomposition (Section~\ref{sec:eda}) confirms that the seasonal component accounts for only 1.8\% of aggregate variance, \textbf{seasonality} remains a meaningful limitation at the individual listing level. The model was trained on a single cross-sectional snapshot and therefore cannot capture intra-year price variation driven by tourist peaks (July--August), school holidays, or major events. A host deploying the model in winter may receive systematically lower price recommendations than during peak summer demand, and a truly robust pricing tool would require a time-series-aware model trained on repeated observations across all seasons.
\end{enumerate}
...
\begin{center}
  ...
  {\small\color{airbnbGrey}
    Data: Inside Airbnb (CC0 licence) $\cdot$
